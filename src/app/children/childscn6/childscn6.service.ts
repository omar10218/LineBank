...
export class Childscn6Service extends BaseService {
  ...
  getTableData(code: JCICCode): JCICTable {
    ...
    if (code === JCICCode.VAM106) {
      ...
      title = '消債條例信用註記資訊(含個別協商)(多筆Z13，MAINCODE A(前協)、B(更生)、C(清算)、E(調解))、8(個別協商)'
    }
    // VAM107
    if (code === JCICCode.VAM107) {
      ...
      title = '銀行公會消金案件債務協商補充註記(多筆Z13，MAINCODE7)'
    }
    // VAM108
    if (code === JCICCode.VAM108) {
      ...
      title = '其它補充註記資訊(多筆Z13，非屬消債條例及銀行公會債務協商之註記資訊)'
    }
    // BAM029
    if (code === JCICCode.BAM029) {
      ...
      title = '新增額度(含應計入DBR22倍金額)及清償資訊(多筆B29)',
        nzScroll = '1500px'
    }
    // BAM501
    if (code === JCICCode.BAM501) {
      ...
      title = '應計入DBR22倍規範之無擔保債務餘額彙總(多筆B68)'
    }
    // BAM502
    if (code === JCICCode.BAM502) {
      ...
      title = '應計入DBR22倍規範之無擔保放款明細(多筆B68)'
    }
    // BAM504
    if (code === JCICCode.BAM504) {
      ...
      title = '應計入DBR22倍規範之貸款餘額(應計入DBR22倍規範之貸款餘額(擔保放款餘額加上部分擔保、負擔保貸款餘額)扣除擔保品鑑估值之金額彙總)'
    }
    // BAM505
    if (code === JCICCode.BAM505) {
      ...
      title = '新增核准額度中應計入DBR22倍之金額明細(多筆B68)'
    }
    // BAM032
    if (code === JCICCode.BAM032) {
      ...
      title = '每日授信餘額變動資訊-餘額增加(含撥款及轉(多筆，每日授信餘額變動資訊-餘額增加(含撥款及轉列為催收呆帳)))'
    }
    // BAM011
    if (code === JCICCode.BAM011) {
      dataList = [
        { name: '交易類別', bodyKey: 'XACT_CODE' },
        { name: '延遲繳款應繳日期/繳清日期', bodyKey: 'PASSDUE_PAYOFF_DATE' },
        { name: '延遲繳款應繳金額', bodyKey: 'PASSDUE_AMT' },
        { name: '行庫代號', bodyKey: 'BANK_CODE' },
        { name: '行庫名稱)', bodyKey: 'BANK_NAME' },
        { name: '帳號', bodyKey: 'ACCOUNT_NUM' },
      ];
      title = '最新延遲繳款及繳清延遲繳款資訊(多筆B33)'
    }
    // BAM067
    ...
    if (code === JCICCode.BAM070) {
      ...
      title = '現金卡日報放款餘額及還款紀錄資訊(新版)(多筆B66)'
    }
    // BAM101
    ...
    if (code === JCICCode.BAM305) {
      ...
      title = '99年版授信保證資料(共同債務)(多筆B33-B36)',
        nzScroll = '1500px'
    }
    // BAM306
    if (code === JCICCode.BAM306) {
      ...
      title = '90年版授信保證資料(從債務)(多筆B33-B36)',
        nzScroll = '1500px'
    }
    // BAM307
    ...
    if (code === JCICCode.BAM608) {
      ...
      title = '債權轉讓資料明細-B33(多筆B33，資料年月自9405開始)',
        nzScroll = '1500px'
    }
    // KRM046
    if (code === JCICCode.KRM046) {
      ...
      title = '信用卡正附卡資訊(本人信用卡)(含雙幣卡)(多筆K21)',
        nzScroll = '1500px'
    }
    // KRM048
    if (code === JCICCode.KRM048) {
      ...
      title = '信用卡戶帳款資訊(週報+12個月)(含雙幣卡)(多筆K34)',
        nzScroll = '1500px'
    }
    // STM022
