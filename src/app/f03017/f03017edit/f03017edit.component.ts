...
export class F03017editComponent implements OnInit {
  ...
  public async updateData(): Promise<void> {
    ...
    await this.f03017Service.oneseve(url, this.jsonObject).subscribe(data => {
      // if (data.rspMsg == '儲存成功') {
      if (this.oriContent == content[0].bkContent) { this.changeContect = false };
      const dialogRefOpen = this.dialog.open(ConfirmComponent, { data: { msgStr: data.rspMsg } })
      dialogRefOpen.afterClosed().subscribe(result => {
        if (!this.changeContect) {
